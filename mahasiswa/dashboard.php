...
$id_mahasiswa = isset($_SESSION['mahasiswa']) ? $_SESSION['mahasiswa'] : null;
...
// Data Presensi
$query = mysqli_query($con, "SELECT COUNT(CASE WHEN pm.status_mahasiswa = 'Hadir' THEN 1 END) AS jumlah_hadir,
    COUNT(CASE WHEN pm.status_mahasiswa = 'Sakit' THEN 1 END) AS jumlah_sakit,
    COUNT(CASE WHEN pm.status_mahasiswa = 'Izin' THEN 1 END) AS jumlah_izin,
    COUNT(CASE WHEN pm.status_mahasiswa = 'Terlambat' THEN 1 END) AS jumlah_terlambat,
    COUNT(CASE WHEN pm.status_mahasiswa = 'Alpa' THEN 1 END) AS jumlah_alpa
    FROM presensi_mahasiswas pm
    INNER JOIN jadwals j ON pm.jadwal_id = j.id_jadwal
    INNER JOIN semesters ON j.semester_id = semesters.id_semester
    WHERE (status_presensi = 'Dikonfirmasi' OR status_presensi = 'Ditolak')
	AND j.hari = '$hari'
    AND semesters.status = 1
    AND pm.mahasiswa_id = '$id_mahasiswa'");

// Mengambil data
$jumlah_hadir = 0; // inisialisasi variabel
$jumlah_sakit = 0; // inisialisasi variabel
$jumlah_izin = 0; // inisialisasi variabel
$jumlah_terlambat = 0; // inisialisasi variabel
$jumlah_alpa = 0; // inisialisasi variabel
...
$sql = mysqli_query($con, "SELECT * FROM mahasiswas WHERE id_mahasiswa = '$id_mahasiswa'");
...
<head>
	...
	<title>Mahasiswa | Aplikasi Absensi</title>
					...
					<ul class="navbar-nav topbar-nav ml-md-auto align-items-center">
						...
						<li class="nav-item dropdown hidden-caret">
							<a class="dropdown-toggle profile-pic" data-toggle="dropdown" href="#" aria-expanded="false">
								<div class="avatar-sm">
									<img src="../assets/img/mahasiswa/<?= $data['foto'] ?>" alt="..." class="avatar-img rounded-circle">
								</div>
							</a>
							<ul class="dropdown-menu dropdown-user animated fadeIn">
								<div class="dropdown-user-scroll scrollbar-outer">
									<li>
										<div class="user-box">
											<div class="avatar-lg"><img src="../assets/img/mahasiswa/<?= $data['foto'] ?>" alt="image profile" class="avatar-img rounded"></div>
											<div class="u-text">
												<h4>
													<?= $data['nm_mahasiswa'] ?>
												</h4>
		...
		<div class="main-panel">
			<div class="content">
				...
				if ($page == 'presensi') {
					...
					include 'modul/jadwal/jadwal_kuliah.php';
				} elseif ($page == 'kompensasi') {
					include 'modul/kompensasi/kompen.php';
				} elseif ($page == 'status') {
					...
					<div class="modal-content">
						...
						if (isset($_POST['changePassword'])) {
							...
							if (password_verify($pass, $passLama)) {
								$stmt = $con->prepare("UPDATE mahasiswas SET password=? WHERE id_mahasiswa=?");
								$stmt->bind_param("si", $newPass, $data['id_mahasiswa']);
								$stmt->execute();
								$stmt->close();
					...
					<div class="modal-content">
						...
						<form action="" method="post" enctype="multipart/form-data">
							<div class="modal-body">
								...
								<div class="form-group">
									...
									<input type="text" name="nama" class="form-control" value="<?= $data['nm_mahasiswa'] ?>">
									<input type="hidden" name="id" value="<?= $data['id_mahasiswa'] ?>">
								</div>
								<div class="form-group">
									<label>Foto Profile</label>
									<p>
										<img src="../assets/img/mahasiswa/<?= $data['foto'] ?>" class="img-thumbnail" style="height: 50px;width: 50px;">
									</p>
						...
						if (isset($_POST['updateProfile'])) {
							...
							if (!empty($_FILES['foto']['name'])) {
								...
								$ganti = mysqli_query($con, "UPDATE mahasiswas SET foto='$gambar' WHERE id_mahasiswa='$id_mahasiswa'");
							}

							// Gunakan parameter binding pada pernyataan SQL untuk keamanan
							$sqlEdit = mysqli_prepare($con, "UPDATE mahasiswas SET nm_mahasiswa=? WHERE id_mahasiswa=?");
							mysqli_stmt_bind_param($sqlEdit, "si", $_POST['nama'], $id_mahasiswa);
