...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
	...
	$id_makul = mysqli_real_escape_string($con, $_GET['id']);

	// Hapus data dengan parameter yang aman
	$del = mysqli_query($con, "DELETE FROM mata_kuliahs WHERE id_makul='$id_makul'");

	if ($del) {
		echo "<script>
            // alert('Data telah dihapus !');
            window.location='?page=master&act=makul';
        </script>";
	} else {
