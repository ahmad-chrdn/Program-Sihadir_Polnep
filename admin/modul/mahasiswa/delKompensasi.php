...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    $id_kompensasi = mysqli_real_escape_string($con, $_GET['id']);

    // Hapus data dengan parameter yang aman
    $del = mysqli_query($con, "DELETE FROM kompensasis WHERE id_kompensasi='$id_kompensasi'");

    if ($del) {
        echo "<script>
            // alert('Data telah dihapus !');
            window.location='?page=mahasiswa&act=kompen';
        </script>";
    } else {
